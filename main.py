...
import math
import matplotlib
matplotlib.use("TkAgg")  # Nastavení backendu matplotlibu pro práci s Tkinterem
...
class Function:
    def __init__(self, name='sphere'):
        # Uchovává název zvolené funkce (např. sphere, ackley...)
        self.name = name.lower()

    # Jednoduchá kvadratická funkce (koule) – globální minimum v [0,0,...,0]
    def sphere(self, x): return np.sum(np.asarray(x)**2)

    # Ackleyho funkce – vícečetná lokální minima, obtížná pro optimalizaci
    ...
    # Rastriginova funkce – silně multimodální, minimum v [0,0,...,0]
    def rastrigin(self, x, A=10):
        x = np.asarray(x); d = x.size
        return A*d + np.sum(x**2 - A*np.cos(2*math.pi*x))

    # Rosenbrockova funkce – tzv. "banánové údolí", minimum v [1,1,...,1]
    def rosenbrock(self, x):
        x = np.asarray(x)
        return np.sum(100*(x[1:]-x[:-1]**2)**2 + (x[:-1]-1)**2)

    # Griewankova funkce – mnoho lokálních minim, ale globální minimum v [0,...,0]
    def griewank(self, x):
        x = np.asarray(x)
    ...
    # Schwefelova funkce – minimum není v centru, ale mimo původ
    def schwefel(self, x):
        x = np.asarray(x); d = x.size
        return 418.9829*d - np.sum(x*np.sin(np.sqrt(np.abs(x))))

    # Levyho funkce – také těžká multimodální funkce
    def levy(self, x):
        x = np.asarray(x)
        w = 1 + (x-1)/4
    ...
    # Michalewiczova funkce – ostré lokální minima, parametr m určuje obtížnost
    def michalewicz(self, x, m=10):
        x = np.asarray(x)
        i = np.arange(1, x.size+1)
        return -np.sum(np.sin(x)*(np.sin(i*x**2/math.pi))**(2*m))

    # Zakharovova funkce – kombinace kvadratických a polynomiálních členů
    def zakharov(self, x):
    ...
    # Hlavní metoda pro vyhodnocení zvolené funkce
...
# -> Zkouší náhodné body a zapamatuje si nejlepší
def blind_search(func: Function, iterations=200, lb=-5, ub=5, seed=0):
    if seed: np.random.seed(seed)
    best = np.random.uniform(lb, ub, size=2)  # počáteční náhodný bod
    best_f = float(func.eval(best))           # hodnota funkce v tom bodě
    history = [(best.copy(), best_f)]         # historie všech bodů
    for _ in range(iterations):
        cand = np.random.uniform(lb, ub, size=2)  # nový náhodný kandidát
        fval = float(func.eval(cand))
        if fval < best_f:                      # pokud je lepší, uložíme ho
            best, best_f = cand, fval
        history.append((cand.copy(), fval))
    return best, history

# Hill Climbing (prohledávání sousedství)
# -> Začneme v náhodném bodě a zkoušíme "sousedy", postupně lezeme dolů k minimu
def hill_climbing(func: Function,
    ...
    current = np.random.uniform(lb, ub, size=dimension)  # startovní bod
    ...
    for _ in range(iterations):
        ...
        for _ in range(max(1, int(k_neighbors))):
            ...
            cand = np.clip(cand, lb, ub)  # omezíni na hranice prostoru
            fval = float(func.eval(cand))
            neighbors.append((cand, fval))
        neighbors.sort(key=lambda t: t[1])  # seřadíme sousedy podle hodnoty
        cand, fval = neighbors[0]  # vezmeme nejlepšího souseda
        if fval < current_f:       # pokud je lepší než aktuální, posuneme se
            current, current_f = cand, fval
        if current_f < best_f:     # aktualizujeme globálně nejlepší
...
def show_in_tk(fig, frame, anim=None, update_fn=None, total_frames=None):
    # Vyčistí starý obsah ve frame a vloží nový graf do Tkinter okna
    ...
    # Přidáme reference, abychom mohli s animací pracovat později
...
def animate_algorithm(root_frame, func_name, lb, ub, algo="blind", iterations=200, seed=42):
    # Spuštění algoritmu a získání historie všech navštívených bodů
    ...
    # Připravíme mřížku hodnot pro vykreslení povrchu funkce
    grid_res = 140
    ...
    # Figure s dvěma subplóty (3D a 2D kontura)
    ...
    # Připravíme data o pozicích a hodnotách
    positions = np.array([h[0] for h in history])
    values = np.array([h[1] for h in history])
    frames = positions.shape[0]

    # Scatter objekty pro animaci
    trail3d = ax3d.scatter([], [], [], s=20, c='red', alpha=0.3)  # stopa
    trail2d = ax2d.scatter([], [], s=20, c='red', alpha=0.3)
    scatter3d = ax3d.scatter([], [], [], s=60, c='red')            # aktuální bod
    scatter2d = ax2d.scatter([], [], s=60, c='red')
    best3d = ax3d.scatter([], [], [], s=120, marker='*', color='gold')  # nejlepší bod
    best2d = ax2d.scatter([], [], s=120, marker='*', color='gold')

    ax3d.view_init(elev=30, azim=-60)

    state = {'idx':0, 'playing':True, 'interval':100}  # stav animace
    ...
    # Update funkce pro každý frame animace
    def update(frame):
        i = int(frame)
        state['idx'] = i

        # Stopa navštívených bodů
        pts = positions[:i+1]
        zs = values[:i+1]
        ...
        # Nejlepší nalezený bod do aktuální chvíle
    ...
    # Posuvník pro ruční přeskakování snímků
    ...
    # Funkce reagující na změnu slideru
    ...
    # Tlačítka Play/Pause a krokování animace
    def on_play_pause():
        if state['playing']:
    ...
    # Ovládání pohledu kamery (3D náhled)
    ...
    # Automatická synchronizace slideru při běžící animaci
    def on_timer(event):
        i = int(state['idx'])
        slider.set(i)
        frame_label.config(text=f"{i+1}/{frames}")
...
# --------------------------
# Vykreslení povrchu bez animace
# --------------------------
...
def main():
    ...
    # Levý panel s menu funkcí a tlačítky
    ...
    # Pravý panel pro vykreslování grafů
